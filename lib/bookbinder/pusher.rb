class Pusher
  ...
  def cf_unmap_route(app, host)
    system "#{cf_binary_path} unmap-route #{app} cfapps.io -n #{host}"
  end

  def cf_stop(app)
    system "#{cf_binary_path} stop #{app}"
  end
  ...
  def cf_start(deploy_target_app)
    ...
    system "#{cf_binary_path} start #{deploy_target_app}"
  end

  def cf_map_route(deploy_target_app, host)
    # map hostname to newly deployed app
    system "#{cf_binary_path} map-route #{deploy_target_app} cfapps.io -n #{host}"
  end
  ...
  def cf_push(deploy_target_app)
    ...
    system "#{cf_binary_path} push #{deploy_target_app} --no-route -m 256M -i 2"
  end
