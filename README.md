...
- **repos**: an array of hashes which specifies which documentation repositories to pull in. Each hash needs to specify:
    ...
    - **subnav_template**: (optional) a label for the template in /subnavs to insert at `<%= yield_for_subnavs %>`, when generating the page.
...
Bookbinder provides three helper functions that can be called from within a .erb file in a doc repo, such as a layout file.
...
`<%= yield_for_subnavs %>` inserts the appropriate template in /subnavs, based on each constituent repositories' `subnav_template:` parameter in config.yml
