...
module Bookbinder
  describe ConfigurationFetcher do
    ...
    it "can read config from a relative path even when I've changed working directory" do
      ...
      Dir.chdir('/tmp') do |tmp|
        expect(config_fetcher.fetch_config).to eq(Configuration.new(logger, {foo: 'bar', 'pdf_index' => nil}))
      end
    end
